    ...
    <main class="dash-body">
      ...
      <div class="center-col">
        <div class="panel map-panel">
          <div class="filter-bar" v-if="hasFilter"><span class="filter-label">当前筛选：</span><span class="filter-chip" v-if="filters.district">{{ filters.district }}<i class="chip-x" @click="clearFilter('district')">×</i></span><span class="filter-chip" v-if="filters.category">{{ filters.category }}<i class="chip-x" @click="clearFilter('category')">×</i></span><span class="filter-chip" v-if="filters.status">{{ filters.status }}<i class="chip-x" @click="clearFilter('status')">×</i></span><span class="filter-chip" v-if="filters.risk">{{ filters.risk }}风险<i class="chip-x" @click="clearFilter('risk')">×</i></span><button class="back-btn" @click="resetAll">返回全国</button></div>
      ...
      <div class="side-col">
        ...
        <div class="panel grow1"><div class="panel-title">实时预警</div><div class="alert-list"><div class="alert-item" v-for="(a, i) in summary.alerts" :key="i"><span class="alert-dot" :class="'lv' + (a.alert_level === '高' ? 1 : a.alert_level === '中' ? 2 : 3)"></span><span class="alert-name">{{ a.person_name }}</span><span class="alert-type">{{ a.alert_type }}</span><span class="alert-time">{{ fmtTime(a.created_at) }}</span></div><div class="empty" v-if="!(summary.alerts && summary.alerts.length)">暂无预警</div></div></div>
      </div>
    </main>
...
.map-panel { flex: 1; min-height: 0; position: relative; }
.map-panel :deep(.chart) { height: 100%; }
.center-bottom { display: flex; gap: 14px; height: 30%; flex-shrink: 0; min-height: 0; }
...
.filter-chip { font-size: 12px; color: var(--accent-cyan); background: rgba(0, 212, 255, 0.12); padding: 2px 8px; border-radius: 3px; }
.chip-x { cursor: pointer; margin-left: 5px; color: var(--text-secondary); } .chip-x:hover { color: #fff; }
...
.map-legend { position: absolute; top: 10px; right: 16px; z-index: 5; display: flex; gap: 12px; background: rgba(6, 20, 40, 0.6); padding: 4px 10px; border-radius: 3px; }
...
.dot.high { background: var(--accent-red); box-shadow: 0 0 6px var(--accent-red); } .dot.mid { background: var(--accent-gold); box-shadow: 0 0 6px var(--accent-gold); } .dot.low { background: var(--accent-cyan); box-shadow: 0 0 6px var(--accent-cyan); }
.map-tip { position: absolute; left: 14px; bottom: 12px; z-index: 5; font-size: 12px; color: var(--accent-cyan); }
.map-tooltip { position: absolute; left: 14px; bottom: 12px; z-index: 5; font-size: 12px; color: var(--text-dim); }
.alert-list { overflow: auto; flex: 1; min-height: 0; }
.alert-item { display: flex; align-items: center; gap: 6px; padding: 7px 4px; border-bottom: 1px dashed rgba(0, 212, 255, 0.15); font-size: 12px; }
.alert-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.alert-dot.lv1 { background: var(--accent-red); box-shadow: 0 0 6px var(--accent-red); } .alert-dot.lv2 { background: var(--accent-gold); box-shadow: 0 0 6px var(--accent-gold); } .alert-dot.lv3 { background: var(--accent-cyan); box-shadow: 0 0 6px var(--accent-cyan); }
.alert-name { color: #fff; font-weight: 600; } .alert-type { color: var(--accent-orange); } .alert-time { margin-left: auto; color: var(--text-dim); font-size: 11px; }
.empty { color: var(--text-dim); text-align: center; padding: 20px; font-size: 12px; }
.dash-footer { position: relative; z-index: 2; display: flex; align-items: center; height: 36px; border-top: 1px solid rgba(0, 212, 255, 0.2); background: linear-gradient(180deg, transparent, rgba(0, 50, 100, 0.3)); padding: 0 20px; }
.footer-label { color: var(--accent-gold); font-size: 13px; font-weight: 600; margin-right: 16px; white-space: nowrap; letter-spacing: 2px; }
.ticker-wrap { flex: 1; overflow: hidden; }
.ticker { display: inline-block; white-space: nowrap; animation: ticker-scroll linear infinite; will-change: transform; }
.ticker-item { font-size: 12px; color: var(--text-secondary); margin-right: 40px; }
.tick-type { padding: 1px 6px; border-radius: 2px; font-size: 11px; margin-right: 6px; }
.tick-alert { background: rgba(255, 91, 91, 0.2); color: var(--accent-red); } .tick-visit { background: rgba(0, 212, 255, 0.2); color: var(--accent-cyan); }
