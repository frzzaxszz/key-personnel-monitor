<template>
  <div class="admin-page">
    <div class="page-head"><h2>人员详情</h2><div class="head-actions"><router-link to="/persons" class="btn">← 返回列表</router-link><router-link to="/dashboard" class="btn">大屏</router-link></div></div>
    <div class="detail-wrap" v-if="person">
      <div class="admin-card profile-card">
        <div class="profile-left">
          <div class="photo-wrap"><img v-if="person.photo" :src="person.photo" class="big-photo" /><div v-else class="big-photo empty">{{ person.name[0] }}</div><button class="btn" style="width: 100%; margin-top: 10px" @click="triggerUpload">上传照片</button><input type="file" ref="fileInput" accept="image/*" hidden @change="uploadPhoto" /></div>
        </div>
        <div class="profile-right">
          ...
          <table class="info-table"><tr><td>身份证号</td><td>{{ person.id_card }}</td><td>性别</td><td>{{ person.gender }}</td></tr><tr><td>年龄</td><td>{{ person.age }}</td><td>手机号</td><td>{{ person.phone }}</td></tr><tr><td>人员类别</td><td>{{ person.category }}</td><td>所属地区</td><td>{{ person.district }}</td></tr><tr><td>街道</td><td>{{ person.street }}</td><td>责任民警</td><td>{{ person.manager }}</td></tr><tr><td>详细地址</td><td colspan="3">{{ person.address }}</td></tr><tr><td>备注</td><td colspan="3">{{ person.notes || '—' }}</td></tr><tr><td>入库时间</td><td>{{ fmtTime(person.created_at) }}</td><td>更新时间</td><td>{{ fmtTime(person.updated_at) }}</td></tr></table>
      ...
      <div class="admin-card" v-if="editMode">
        ...
        <div class="form-grid">
          <label>姓名 *<input v-model="form.name" class="input" /></label><label>身份证号 *<input v-model="form.id_card" class="input" /></label>
          <label>性别<select v-model="form.gender" class="select"><option>男</option><option>女</option></select></label><label>年龄<input v-model.number="form.age" type="number" class="input" /></label>
          <label>手机号<input v-model="form.phone" class="input" /></label><label>人员类别<select v-model="form.category" class="select"><option v-for="c in options.categories" :key="c">{{ c }}</option></select></label>
          <label>风险等级<select v-model="form.risk_level" class="select"><option v-for="r in options.risk_levels" :key="r">{{ r }}</option></select></label><label>管控状态<select v-model="form.control_status" class="select"><option v-for="s in options.control_statuses" :key="s">{{ s }}</option></select></label>
          <label>所属地区<input v-model="form.district" class="input" /></label><label>街道<input v-model="form.street" class="input" /></label><label>详细地址<input v-model="form.address" class="input" /></label><label>责任民警<input v-model="form.manager" class="input" /></label>
      ...
      <div class="admin-card">
        ...
        <div class="visit-form"><textarea v-model="visitContent" class="input" placeholder="填写走访记录内容…" rows="2" style="flex: 1"></textarea><button class="btn btn-primary" @click="addVisit">添加记录</button></div>
        <table class="grid-table" style="margin-top: 12px"><thead><tr><th>时间</th><th>内容</th><th>责任民警</th></tr></thead><tbody><tr v-for="v in visits" :key="v.id"><td>{{ fmtTime(v.visit_date) }}</td><td>{{ v.content }}</td><td>{{ v.manager }}</td></tr><tr v-if="!visits.length"><td colspan="3" class="empty">暂无记录</td></tr></tbody></table>
      </div>
    </div>
...
<style scoped>
.page-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; } .page-head h2 { font-size: 20px; }
