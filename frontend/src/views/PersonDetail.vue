    ...
    <div class="detail-wrap" v-if="person">
      <div class="admin-card profile-card">
        ...
        <div class="profile-right">
          <h3 class="p-name">{{ person.name }}
            <span class="tag" :class="riskClass(person.risk_level)">风险 {{ person.risk_level }}</span>
            <span class="tag" :class="statusClass(person.control_status)">{{ person.control_status }}</span>
          </h3>
          ...
          <div class="profile-actions">
            <button class="btn btn-primary" @click="editMode = !editMode">{{ editMode ? '取消编辑' : '编辑信息' }}</button>
          </div>
        </div>
      </div>
        ...
        <table class="grid-table" style="margin-top: 12px">
          ...
          <tbody>
            <tr v-for="v in visits" :key="v.id">
              <td>{{ fmtTime(v.visit_date) }}</td><td>{{ v.content }}</td><td>{{ v.manager }}</td>
            </tr>
            <tr v-if="!visits.length"><td colspan="3" class="empty">暂无记录</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
async function load() {
  ...
  person.value = await personApi.get(id)
  visits.value = await personApi.visits(id)
  Object.assign(form, JSON.parse(JSON.stringify(person.value)))
}
...
async function uploadPhoto(e) {
  ...
  if (!file) return
  await personApi.uploadPhoto(person.value.id, file)
  await load()
}

async function saveEdit() {
  await personApi.update(person.value.id, form)
  editMode.value = false
  await load()
}

async function addVisit() {
  if (!visitContent.value.trim()) return
  await personApi.addVisit(person.value.id, { content: visitContent.value, manager: form.manager || '系统' })
  visitContent.value = ''
  visits.value = await personApi.visits(person.value.id)
}

onMounted(async () => {
  options.value = await personApi.options()
  load()
})
</script>
...
.detail-wrap { display: flex; flex-direction: column; gap: 16px; max-width: 1080px; margin: 0 auto; width: 100%; }
.profile-card { display: flex; gap: 24px; }
.profile-left { width: 180px; }
.big-photo {
  width: 180px; height: 220px; border-radius: 8px; object-fit: cover;
  border: 1px solid rgba(0, 212, 255, 0.5);
}
.big-photo.empty {
  display: flex; align-items: center; justify-content: center;
  font-size: 60px; font-weight: 700; color: var(--accent-cyan);
